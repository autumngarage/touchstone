...
- **Stop when the task is correct, not when review runs out of remarks** — deterministic gates first (build, tests, linters, static analysis), then review. Implement only high-severity findings: correctness, crashes, data loss, security, broken behaviour, unacceptable performance, lifecycle failures. Answer and route the rest. Then at most **one** re-review, to confirm no regression — never to reopen the design space. A reviewer always has another remark; that is not a finish line.
...
6. **Ship.** `git push -u origin HEAD`, then `gh pr create`. Put the configured closing reference (`Closes #123` or `Fixes AUT-123`) in the **PR body**, not only a commit. **Review starts by itself when the pull request opens** — do not request one, and never type a request marker by hand: it carries no base coordinates, fails the binding check closed, and wedges the sequencer that would repair it. Re-review a later head by re-running the PR-open command, which is idempotent.
