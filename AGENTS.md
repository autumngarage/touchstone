...
5. **Local review — the tier's one pass, and its evidence.** normal: `coderabbit review --agent --uncommitted` on the staged slice before commit; serious: `codex review --base <default>` on the branch before push. Record reviewer, head, and finding count in the PR body's `- Local review:` row (or `n/a — <reason>` when the CLI is absent or out of quota); `delivery-evidence` refuses a normal or serious PR without it.
