...
4. **Change + commit.** Stage explicit files. Normal: run `touchstone review check`, then `codex -p review-normal review --uncommitted`; failure is a waiver, not permission for default-profile fallback. One concern per commit. Run `git show --stat --oneline HEAD`; unchanged `HEAD`: do not ship.
