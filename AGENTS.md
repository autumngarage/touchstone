...
5. **Local review handoff.** Normal: `openrouter on staged: <n> findings`. Serious: save `git rev-parse HEAD`; run `codex review --base <default>` once; record `codex on <head-sha>: <n> findings, <disposition>` or Codex-unavailable waiver; never record the base. Hosted review owns exact heads.
