...
5. **Local review handoff.** Serious: `codex review --base <default>` once pre-push and may waive only when Codex is unavailable. Record pass/waiver in the PR body. After local fixes, rerun deterministic checks and push. Hosted PR review owns every pushed exact head; never run a confirming local pass.
...
9. **Merge.** After fresh exact-head success, `touchstone pr merge <n> --head <reviewed-sha>` queues or merges and returns; never starts review. Raw recovery: verify policy-bound exact-head gate success postdates all PR review activity, then `gh pr merge <n> --squash --match-head-commit <sha>`. Confirm GitHub state.
