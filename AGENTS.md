...
3. **Codex hook safety.** `hooks/codex-review.sh` runs during `git push`. A bug here can block or silently skip all pushes. The fail-open design (graceful skip on errors) must be preserved. **AI review is advisory** — it does not replace deterministic checks (lint, tests, type checking). The hook fails open by default (`on_error=fail-open`): infra errors (timeout, missing provider, malformed output) allow the push rather than blocking it. Each fail-open event emits a visible `[fail-open:<code>]` line to stderr and writes a structured entry to `~/.touchstone-review-log`; the four taxonomy codes are `FAIL_OPEN_TIMEOUT`, `FAIL_OPEN_PARSE_ERROR`, `FAIL_OPEN_DEPENDENCY_MISSING`, and `FAIL_OPEN_PROVIDER_UNAVAILABLE`. The trade-off is explicit: a Conductor outage during a critical week will not block pushes, but the AI safety net will be visibly absent rather than silently bypassed.
