...
- Reconcile configured-tracker state before opening the PR. Touchstone fixes
  ...
  squash merge.
- Ship with `git push -u origin HEAD`, then use `bash bin/touchstone pr open` with the reviewed title and body, and `--expect-branch "$(git branch --show-current)"` so a worktree cannot open the pull request for a different branch; merge with `bash bin/touchstone pr merge <n> --head <reviewed-sha>`. The source commands sequence GitHub and verify surviving state; `principles/git-workflow.md` carries their raw recovery equivalents.
