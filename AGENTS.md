...
4. **Change, then the tier's one local review pass.** Stage explicit file paths; normal: `coderabbit review --agent --uncommitted` on the staged slice (before the commit, or it sees nothing); serious: `codex review --base <default>` on the branch before the push. Record reviewer, head, and finding count in the PR body's `- Local review:` row (`codex on abc1234: 3 findings, 2 fixed, 1 routed`), or `n/a — <reason>` only when the reviewer CLI is not installed, not authenticated, or out of quota; `delivery-evidence` refuses a normal or serious PR without it.
5. **Commit.** Concise message. One concern per commit.
