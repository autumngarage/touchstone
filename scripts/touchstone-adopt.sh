...
for component in compiler planner transaction parsers workspaces node-locks node-package node python-deps python-uv python-evidence python native-swift native-rust native-go; do
