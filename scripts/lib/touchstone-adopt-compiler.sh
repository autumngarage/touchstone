...
record_target() {
  local name="$1" path="$2" profile="$3" lookup_status
  ...
  case "$path$profile" in *"$TAB"* | *"$CR"* | *"$LF"*)
    contract_refusal "target '$name' contains a plan-record delimiter"
    ;;
  esac
  ...
  [ "$lookup_status" -eq 1 ] \
    || operational_failure "could not inspect adoption targets before recording '$name'"
  printf '%s\t%s\t%s\n' "$name" "$path" "$profile" >>"$TARGETS_FILE" \
    || operational_failure "could not record adoption target '$name'"
}

record_task() {
  local name="$1" target="$2" command="$3" lookup_status
  valid_identifier "$name" || contract_refusal "invalid task name '$name'"
  valid_identifier "$target" || contract_refusal "task '$name' has an invalid target '$target'"
  ...
  [ "$lookup_status" -eq 1 ] \
    || operational_failure "could not inspect adoption tasks before recording '$name'"
...
record_setup() {
  ...
  case "$relative" in *"$TAB"* | *"$CR"* | *"$LF"*)
    contract_refusal "setup path contains a plan-record delimiter"
    ;;
  esac
  [ -n "$(trim "$command")" ] \
...
profile_has_tasks() {
  local directory="$1" profile="$2" task probe_status
  ...
  case "$profile" in
    ...
    python)
      ...
      if [ -f "$directory/pyproject.toml" ]; then
        if grep -Eq '^\[tool\.(ruff|mypy|pytest)(\.|\])' "$directory/pyproject.toml"; then
          return 0
        else
          probe_status=$?
        fi
        [ "$probe_status" -eq 1 ] \
          || operational_failure "could not inspect Python task declarations in ${directory#"$PROJECT_ROOT"/}/pyproject.toml"
      fi
