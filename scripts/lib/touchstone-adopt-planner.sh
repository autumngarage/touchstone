...
render_consumer_markdown() {
  ...
  sed \
    ...
    -e 's#^Under the hood this uses the same GitHub API flow (claim + dispatch comment), equivalent to:$#Complete the claim with a dispatch comment, then re-read the assignee:#' \
    -e 's#principles/#.touchstone/principles/#g' \
    "$source" >"$output" || operational_failure "could not render consumer steering"
  if grep -Eq 'bash scripts/(claim-issue|respond-review)\.sh' "$output"; then
    operational_failure "consumer steering retained a repository-local delivery command"
  fi
}
