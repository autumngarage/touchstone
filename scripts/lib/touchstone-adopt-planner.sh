...
plan_managed_file() {
  ...
  if [ -e "$PROJECT_ROOT/$relative" ]; then
    ...
    git -C "$PROJECT_ROOT" ls-files --error-unmatch -- "$relative" >/dev/null 2>&1 \
      || contract_refusal "Touchstone-managed output '$relative' is not tracked; commit it or remove it before planning"
  fi
...
require_tracked_directory_fact() {
  local path="$1"
  [ ! -d "$PROJECT_ROOT/$path" ] || git -C "$PROJECT_ROOT" ls-files -- "$path" | grep -q . \
    || contract_refusal "adoption compiler directory fact '$path' has no tracked contents; commit it or remove it before planning"
}
...
require_compiler_inputs_tracked() {
  ...
  require_tracked_directory_fact tests
...
compile_plan() {
  ...
  if [ -f "$PROJECT_ROOT/.touchstone.toml" ]; then
    ...
    require_tracked_compiler_path .touchstone.toml
    validate_existing_contract
    PROFILE="declared-v1"
