...
abort_staging() {
  local temporary="$1" stage_file="$2" applied_file="$3" directories_file="$4" context="$5" primary="$6"
  local staged_cleanup_failed=false transaction_cleanup_failed=false
  rm -f "$temporary" || staged_cleanup_failed=true
  cleanup_apply_artifacts "$stage_file" "$applied_file" "$directories_file" \
    || transaction_cleanup_failed=true
  if [ "$staged_cleanup_failed" = true ] && [ "$transaction_cleanup_failed" = true ]; then
    operational_failure "could not remove the staged write or clean transaction files after $context; original failure: $primary"
  fi
  [ "$staged_cleanup_failed" = false ] \
    || operational_failure "could not remove the staged write after $context; original failure: $primary"
  [ "$transaction_cleanup_failed" = false ] \
    || operational_failure "could not clean transaction files after $context; original failure: $primary"
  operational_failure "$primary"
}

apply_plan() {
  local action path ownership destination source old_source parent temporary worktree_status default_status backup backup_pending backup_directory backup_cleanup_failed
  ...
  : >"$stage_file" || operational_failure "could not initialize staged adoption writes"
  : >"$applied_file" || operational_failure "could not initialize applied adoption writes"
  : >"$directories_file" || operational_failure "could not initialize adoption directory tracking"
  APPLY_ACTIVE=true
  while IFS="$(printf '\t')" read -r action path ownership; do
    ...
    if [ "$action" = update ]; then
      [ -f "$destination" ] && [ ! -L "$destination" ] || {
        abort_staging "$temporary" "$stage_file" "$applied_file" "$directories_file" \
          "$path changed during staging" "$path changed during staging"
      }
      if ! expected_hash="$(git hash-object "$old_source")"; then
        abort_staging "$temporary" "$stage_file" "$applied_file" "$directories_file" \
          "snapshotting $path failed" "could not read the planned snapshot for $path"
      fi
      if ! expected_mode="$(LC_ALL=C ls -ld "$old_source" | awk '{ print $1 }')"; then
        abort_staging "$temporary" "$stage_file" "$applied_file" "$directories_file" \
          "snapshotting $path metadata failed" "could not read the planned metadata for $path"
      fi
      if ! current_hash="$(git hash-object "$destination")" \
        || ! current_mode="$(LC_ALL=C ls -ld "$destination" | awk '{ print $1 }')"; then
        abort_staging "$temporary" "$stage_file" "$applied_file" "$directories_file" \
          "rechecking $path failed" "could not recheck $path before apply"
      fi
      if [ "$current_hash" != "$expected_hash" ] || [ "$current_mode" != "$expected_mode" ]; then
        abort_staging "$temporary" "$stage_file" "$applied_file" "$directories_file" \
          "$path changed since planning" "$path changed since planning"
      fi
    fi
    if ! printf '%s\t%s\t%s\t%s\t%s\n' "$action" "$destination" "$temporary" "$expected_hash" "$expected_mode" >>"$stage_file"; then
      abort_staging "$temporary" "$stage_file" "$applied_file" "$directories_file" \
        "recording $path failed" "could not record staged write for $path"
    fi
    if [ -e "$destination" ]; then
      if ! cp -p "$destination" "$temporary" || ! cat "$source" >"$temporary"; then
        abort_staging "$temporary" "$stage_file" "$applied_file" "$directories_file" \
          "staging $path failed" "could not stage $path"
      fi
  ...
  while IFS="$(printf '\t')" read -r action destination temporary expected_hash expected_mode; do
    ...
    if [ "$action" = update ]; then
      ...
      backup_directory="$(mktemp -d "$(dirname "$destination")/.touchstone-backup.XXXXXX")" || {
        rollback_apply "$applied_file" "$stage_file" "$directories_file" \
      ...
      backup_pending="$backup_directory/pending"
    ...
    if ! printf '%s\t%s\t%s\t%s\n' "$action" "$destination" "$backup" "$backup_directory" >>"$applied_file"; then
      backup_cleanup_failed=false
      if [ "$backup_directory" != - ] && ! rmdir "$backup_directory"; then backup_cleanup_failed=true; fi
      rollback_apply "$applied_file" "$stage_file" "$directories_file" \
        || operational_failure "could not roll back after transaction recording failed for $path"
      [ "$backup_cleanup_failed" = false ] \
        || operational_failure "could not clean rollback directory after transaction recording failed for $path"
      operational_failure "could not record applied write for $path"
    fi
    if [ "$action" = update ]; then
      if ! cp -p "$destination" "$backup_pending" || ! mv "$backup_pending" "$backup"; then
        backup_cleanup_failed=false
        if ! rm -f "$backup_pending" "$backup"; then backup_cleanup_failed=true; fi
        rollback_apply "$applied_file" "$stage_file" "$directories_file" \
          || operational_failure "could not roll back after backing up $path failed"
        [ "$backup_cleanup_failed" = false ] \
          || operational_failure "rolled back after backing up $path failed, but could not clean incomplete rollback data"
        operational_failure "could not preserve rollback data for $path"
      fi
    fi
...
record_missing_directories() {
  ...
  while IFS= read -r entry; do
    [ -n "$entry" ] || continue
    if ! grep -Fqx "$entry" "$output" 2>/dev/null; then
      printf '%s\n' "$entry" >>"$output" \
        || operational_failure "could not record adoption directory $entry"
    fi
  done < <(printf '%s\n' "$missing" | awk '{ lines[NR] = $0 } END { for (line = NR; line >= 1; line--) print lines[line] }')
}

cleanup_apply_artifacts() {
  local stage_file="$1" applied_file="$2" directories_file="$3" preserve_backups="${4:-false}"
  local action destination artifact backup_directory expected_hash expected_mode failed=false directory
  ...
  if [ "$preserve_backups" != true ] && [ -f "$applied_file" ]; then
    while IFS="$(printf '\t')" read -r action destination artifact backup_directory; do
      [ "${artifact:-}" = - ] || [ -z "${artifact:-}" ] || [ ! -e "$artifact" ] || rm -f "$artifact" || failed=true
      [ "${backup_directory:-}" = - ] || [ -z "${backup_directory:-}" ] \
        || [ ! -d "$backup_directory" ] || rmdir "$backup_directory" || failed=true
    done <"$applied_file"
  fi
...
rollback_apply() {
  local applied_file="$1" stage_file="$2" directories_file="$3" action destination backup backup_directory failed=false
  if [ -f "$applied_file" ]; then
    while IFS="$(printf '\t')" read -r action destination backup backup_directory; do
  ...
  cleanup_apply_artifacts "$stage_file" "$applied_file" "$directories_file" "$failed" || failed=true
