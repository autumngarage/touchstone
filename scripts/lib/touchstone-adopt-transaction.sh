...
apply_plan() {
  ...
  while IFS="$(printf '\t')" read -r action destination temporary expected_hash expected_mode; do
    ...
    if [ "$action" = update ]; then
      ...
      if ! rm -f "$backup"; then
        rollback_apply "$applied_file" "$stage_file" "$directories_file" \
          || operational_failure "could not roll back after preparing rollback data for $path failed"
        operational_failure "could not prepare rollback path for $path"
      fi
...
cleanup_apply_artifacts() {
  ...
  local action destination artifact expected_hash expected_mode failed=false directory
