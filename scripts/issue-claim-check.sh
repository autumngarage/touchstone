...
extract_issue_refs() {
  ...
  done < <(grep -Eoi '\b(close|closes|closed|fix|fixes|fixed|resolve|resolves|resolved|closes-issue):?[[:space:]]*#[0-9]+\b' "$body_file" || true)

  # Pattern 2: GitHub closing keywords with optional owner/repo prefix.
  while IFS= read -r match; do
    normalized="$(printf '%s' "$match" | tr '[:upper:]' '[:lower:]')"
    target_repo="$(printf '%s' "$normalized" | sed -nE 's/^(close|closes|closed|fix|fixes|fixed|resolve|resolves|resolved):?[[:space:]]*([[:alnum:]_.-]+\/[[:alnum:]_.-]+)#[0-9]+$/\2/p')"
  ...
  done < <(grep -Eoi '\b(close|closes|closed|fix|fixes|fixed|resolve|resolves|resolved):?[[:space:]]*([[:alnum:]_.-]+/[[:alnum:]_.-]+)?#[0-9]+\b' "$body_file" || true)
}
