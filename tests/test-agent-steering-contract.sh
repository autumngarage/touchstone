...
[ ! -e "$TOUCHSTONE_ROOT/scripts/touchstone-review.sh" ] \
  || fail "the retired local review script still ships"
[ ! -e "$TOUCHSTONE_ROOT/config/review-normal.json" ] \
  || fail "the retired local review policy still ships"
assert_not_contains "$TOUCHSTONE_ROOT/scripts/touchstone-steering-install.sh" 'touchstone-review.sh'
assert_not_contains "$TOUCHSTONE_ROOT/scripts/touchstone-steering-install.sh" 'review setup'
assert_not_contains "$TOUCHSTONE_ROOT/bin/touchstone" 'touchstone review setup|check|run|rotate|uninstall'
...
# A stale caller -- an older daemon, a vendored shipping script, an agent on
# cached steering -- fails loudly with the reason and the credential cleanup,
# exits 2, prints nothing a parser could take for a result, and reaches no
# credential, reviewer, or network on the way.
RETIRED_BIN="$TEST_DIR/retired-review-bin"
RETIRED_CALLS="$TEST_DIR/retired-review-calls"
mkdir -p "$RETIRED_BIN"
for tool in curl security codex jq git; do
  cat >"$RETIRED_BIN/$tool" <<EOF
#!/usr/bin/env bash
printf '%s\n' "$tool" >>"$RETIRED_CALLS"
EOF
  chmod +x "$RETIRED_BIN/$tool"
done
for retired_args in "run" "run --base origin/main" "run --json" "check" "setup" "rotate" "uninstall" ""; do
  retired_status=0
  # shellcheck disable=SC2086 # word-splitting the argument list is the point
  PATH="$RETIRED_BIN:$PATH" bash "$TOUCHSTONE_ROOT/bin/touchstone" review $retired_args \
    >"$TEST_DIR/retired-review.out" 2>"$TEST_DIR/retired-review.err" || retired_status=$?
  [ "$retired_status" -eq 2 ] \
    || fail "touchstone review $retired_args exited $retired_status, not 2"
  [ ! -s "$TEST_DIR/retired-review.out" ] \
    || fail "touchstone review $retired_args wrote to stdout: $(cat "$TEST_DIR/retired-review.out")"
  assert_contains "$TEST_DIR/retired-review.err" "'touchstone review' is retired (AUT-1962)"
  assert_contains "$TEST_DIR/retired-review.err" 'security delete-generic-password -s com.autumngarage.touchstone.review-normal'
done
[ ! -e "$RETIRED_CALLS" ] \
  || fail "the retired review command invoked: $(tr '\n' ' ' <"$RETIRED_CALLS")"
...
assert_contains "$TOUCHSTONE_ROOT/docs/product-contract.md" \
  "each run evaluates once and never waits"
assert_not_contains "$TOUCHSTONE_ROOT/docs/product-contract.md" \
