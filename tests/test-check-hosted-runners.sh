#!/usr/bin/env bash
# The hosted-runner check (AUT-1592): the shared validate workflow refuses a
# GitHub-hosted macOS or Windows runner, and a setting-derived selector that
# does not require the self-hosted label, in any consumer workflow.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd -P)"
CHECK="$ROOT/scripts/check-hosted-runners.sh"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
...
# AUT-1595's shape for the Touchstone required workflows.
dir="$(project)"
workflow "$dir" <<'EOF'
jobs:
  validate:
...
if grep -Fq 'c.yml:' <<<"$out"; then
  fail "a clean file was blamed for a selector left open in another: $out"
fi
ok "a selector open at the end of one file is not carried into the next"
...
echo "==> Touchstone's own workflows"
expect 0 "Touchstone names no hosted macOS or Windows runner" "$ROOT"

echo "All hosted-runner check tests passed."
