...
#!/usr/bin/env bash
printf '%s\n' "\$*" >> "$HOOKS_LOG"
# Write the pre-commit-framework marker so touchstone doctor --project can
# verify the shim is the real pre-commit-framework install, not a stub from
# another framework silently replacing the file.
case "\$*" in
...
# failure as hook files being absent outright.
