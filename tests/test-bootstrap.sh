...
assert_exists "$PROJECT/principles/memory-hygiene.md"
...
assert_contains "$PROJECT/.touchstone-config" '^sync_ship=true$'
assert_contains "$PROJECT/.touchstone-config" '^validate_lane=auto$'
assert_contains "$PROJECT/.touchstone-config" '^validate_affected_command=$'
assert_contains "$PROJECT/.touchstone-config" '^validate_smoke_command=$'
assert_contains "$PROJECT/.touchstone-config" '^validate_full_command=$'
...
#!/usr/bin/env bash
printf '%s\n' "\$*" >> "$HOOKS_LOG"
# Write a TYPED pre-commit-framework shim (marker + the framework's
# hook-impl --hook-type line) and chmod +x, mirroring what a real
# typed pre-commit install produces. Doctor's readiness predicate
# validates the recorded type against the slot the file occupies, so a
# marker-only fake would fail as an untyped shim.
case "\$*" in
...
if (cd "$PROJECT_DOCTOR_HOOK_DRIFT" && TOUCHSTONE_NO_AUTO_UPDATE=1 "$TOUCHSTONE_ROOT/bin/touchstone" doctor --project) >"$TEST_DIR/doctor-hook-drift.txt" 2>&1; then
  ...
  assert_contains "$TEST_DIR/doctor-hook-drift.txt" "aren't typed pre-commit-framework shims"
fi

# doctor must flag a pre-commit shim copied into the pre-push slot: the
# generic framework marker is present and the file is executable, but the
# shim runs --hook-type pre-commit, so pre-push validation is silently
# bypassed (issue #515 acceptance: readiness must be typed).
PROJECT_DOCTOR_HOOK_WRONG_TYPE="$TEST_DIR/test-project-doctor-hook-wrong-type"
PATH="$HOOKS_FAKE_BIN:$PATH" bash "$TOUCHSTONE_ROOT/bootstrap/new-project.sh" "$PROJECT_DOCTOR_HOOK_WRONG_TYPE" --no-register >/dev/null
cp "$PROJECT_DOCTOR_HOOK_WRONG_TYPE/.git/hooks/pre-commit" "$PROJECT_DOCTOR_HOOK_WRONG_TYPE/.git/hooks/pre-push"
if (cd "$PROJECT_DOCTOR_HOOK_WRONG_TYPE" && TOUCHSTONE_NO_AUTO_UPDATE=1 "$TOUCHSTONE_ROOT/bin/touchstone" doctor --project) >"$TEST_DIR/doctor-hook-wrong-type.txt" 2>&1; then
  echo "FAIL: doctor --project should exit nonzero when pre-push runs the wrong --hook-type" >&2
  ERRORS=$((ERRORS + 1))
else
...
# -----------------------------------------------------------------------------
# Typed hook-readiness predicate (issue #515). Doctor, init's first-push gate,
# and update --ship all share touchstone_project_hooks_ready; these fixtures
# pin its contract directly:
#   1. correct shims          → ready
#   2. copied wrong type      → NOT ready (marker alone must not authorize)
...
#   8. alternate --config     → NOT ready (must bind to .pre-commit-config.yaml)
# -----------------------------------------------------------------------------
phase "typed hook-readiness predicate"

# shellcheck source=../lib/install-hooks.sh
source "$TOUCHSTONE_ROOT/lib/install-hooks.sh"

readiness_repo() {
  local dir="$TEST_DIR/readiness-$1"
  mkdir -p "$dir"
...
readiness_write_shim() {
  local path="$1" hook_type="$2" config="${3:-.pre-commit-config.yaml}"
  cat >"$path" <<EOF
#!/usr/bin/env bash
# File generated by pre-commit: https://pre-commit.com
...
READY_R1="$(readiness_repo case1)"
readiness_write_shim "$READY_R1/.git/hooks/pre-commit" pre-commit
readiness_write_shim "$READY_R1/.git/hooks/pre-push" pre-push
if ! touchstone_project_hooks_ready "$READY_R1"; then
  echo "FAIL: readiness 1 — correctly typed shims must report ready" >&2
  ERRORS=$((ERRORS + 1))
fi

READY_R2="$(readiness_repo case2)"
readiness_write_shim "$READY_R2/.git/hooks/pre-commit" pre-commit
readiness_write_shim "$READY_R2/.git/hooks/pre-push" pre-commit
if touchstone_project_hooks_ready "$READY_R2" \
  || touchstone_pre_commit_hook_ready "$READY_R2/.git/hooks/pre-push" pre-push; then
  echo "FAIL: readiness 2 — wrong-typed pre-push shim must not report ready" >&2
  ERRORS=$((ERRORS + 1))
fi

READY_R3="$(readiness_repo case3)"
printf '#!/bin/sh\nexit 0\n' >"$READY_R3/.git/hooks/pre-commit"
chmod +x "$READY_R3/.git/hooks/pre-commit"
readiness_write_shim "$READY_R3/.git/hooks/pre-push" pre-push
if touchstone_project_hooks_ready "$READY_R3"; then
  echo "FAIL: readiness 3 — inert pre-commit hook must not report ready" >&2
  ERRORS=$((ERRORS + 1))
fi

READY_R4="$(readiness_repo case4)"
readiness_write_shim "$READY_R4/.git/hooks/pre-commit" pre-commit
if touchstone_project_hooks_ready "$READY_R4" \
  || touchstone_project_hooks_present "$READY_R4"; then
  echo "FAIL: readiness 4 — missing pre-push hook must not report ready/present" >&2
  ERRORS=$((ERRORS + 1))
fi

READY_R5="$(readiness_repo case5)"
readiness_write_shim "$READY_R5/.git/hooks/pre-commit" pre-commit
readiness_write_shim "$READY_R5/.git/hooks/pre-push" pre-push
chmod -x "$READY_R5/.git/hooks/pre-push"
if touchstone_project_hooks_ready "$READY_R5"; then
  echo "FAIL: readiness 5 — non-executable pre-push hook must not report ready" >&2
  ERRORS=$((ERRORS + 1))
fi

READY_R6="$(readiness_repo case6)"
readiness_write_shim "$READY_R6/.git/hooks/pre-commit" pre-commit
readiness_write_shim "$READY_R6/.git/hooks/pre-push" pre-push
READY_R6_WT="$TEST_DIR/readiness-case6-linked"
git -C "$READY_R6" worktree add -q -b feature/linked "$READY_R6_WT"
if ! touchstone_project_hooks_ready "$READY_R6_WT"; then
  echo "FAIL: readiness 6 — linked worktree must resolve shared hooks as ready" >&2
  ERRORS=$((ERRORS + 1))
fi
READY_R6_RESOLVED="$(touchstone_git_hook_path "$READY_R6_WT" pre-push)"
# macOS tmpdirs symlink /var → /private/var, so compare by suffix: the
# resolved path must land in the PRIMARY checkout's git dir.
case "$READY_R6_RESOLVED" in
  */readiness-case6/.git/hooks/pre-push | */readiness-case6/.git/worktrees/*) ;;
  *)
    echo "FAIL: readiness 6 — unexpected resolved pre-push path: $READY_R6_RESOLVED" >&2
    ERRORS=$((ERRORS + 1))
    ;;
esac

READY_R7="$(readiness_repo case7)"
mkdir -p "$READY_R7/custom-hooks"
git -C "$READY_R7" config core.hooksPath custom-hooks
readiness_write_shim "$READY_R7/custom-hooks/pre-commit" pre-commit
readiness_write_shim "$READY_R7/custom-hooks/pre-push" pre-push
if ! touchstone_project_hooks_ready "$READY_R7"; then
  echo "FAIL: readiness 7 — core.hooksPath shims must report ready" >&2
  ERRORS=$((ERRORS + 1))
fi
READY_R7_RESOLVED="$(touchstone_git_hook_path "$READY_R7" pre-commit)"
case "$READY_R7_RESOLVED" in
  */custom-hooks/pre-commit) ;;
  *)
    echo "FAIL: readiness 7 — resolution must honor core.hooksPath, got: $READY_R7_RESOLVED" >&2
    ERRORS=$((ERRORS + 1))
    ;;
esac
...
if touchstone_project_hooks_ready "$READY_R8" \
  || touchstone_pre_commit_hook_ready "$READY_R8/.git/hooks/pre-commit" pre-commit; then
  echo "FAIL: readiness 8 — shim bound to an alternate config must not report ready" >&2
  ERRORS=$((ERRORS + 1))
fi
