...
# that script sources — and preflight itself sources sha256/toml, while the
# claim scripts source issue-tracker. Keeping that list here means a future
# module addition updates one place instead of every harness.
#
...
stage_touchstone_libs() {
  ...
  mkdir -p "$lib_dir"
  for module in preflight.sh sha256.sh toml.sh events.sh issue-tracker.sh; do
