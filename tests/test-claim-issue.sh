...
EOF
  chmod +x "$TMP/bin/gh"
  export PATH="$TMP/bin:$PATH" GH_REPO="autumngarage/current" GH_CALLS="$TMP/gh-calls" GH_STATE="$TMP/gh-state"
...
EOF
  ...
  assert_json "$TMP/out"
  for option in --body-file --disposition; do
  ...
  run_adapter "$TMP/out" validate 42 --note-file "$TMP/note" --json
  assert_rc "$RUN_RC" 2
  assert_has "$TMP/out" '"reason":"unknown-argument"'
  ...
  run_adapter "$TMP/out" validate AUT-281 --disposition fixed --body-file "$TMP/body" --project "$TMP/linear"
  assert_rc "$RUN_RC" 2
  assert_has "$TMP/out" "Replace 'Closes #281' with 'Fixes AUT-281'"
  printf '%s\n' 'Fixes AUT-281' >"$TMP/body"
  run_adapter "$TMP/out" validate 42 --disposition fixed --body-file "$TMP/body" --project "$TMP/github"
  assert_rc "$RUN_RC" 2
  ...
  run_adapter "$TMP/out" validate 42 --disposition fixed --body-file "$TMP/large-body" --project "$TMP/github" --json
  assert_rc "$RUN_RC" 2
  assert_has "$TMP/out" '"reason":"missing-closing-reference"'
  assert_json "$TMP/out"
  ...
  assert_rc "$RUN_RC" 0
  assert_not_has "$TMP/out" 'wrong-tracker-closing-syntax'
  GH_REPO='' run_adapter "$TMP/out" validate AUT-281 --disposition fixed --body-file "$TMP/body" --project "$TMP/linear" --json
  assert_rc "$RUN_RC" 0
  assert_not_has "$TMP/out" 'wrong-tracker-closing-syntax'
  GH_REPO='' GH_MODE=repo_fail run_adapter "$TMP/out" validate AUT-281 --disposition fixed --body-file "$TMP/body" --project "$TMP/unresolved-linear" --json
  assert_rc "$RUN_RC" 0
  assert_not_has "$TMP/out" 'wrong-tracker-closing-syntax'
  ...
  GH_REPO='github.example.com/autumngarage/current' run_adapter "$TMP/out" validate AUT-281 --disposition fixed --body-file "$TMP/body" --project "$TMP/linear"
  assert_rc "$RUN_RC" 2
  assert_has "$TMP/out" 'qualify cross-repository GitHub closes'
  ...
  run_adapter "$TMP/out" validate 42 --disposition stale --body-file "$TMP/body" --project "$TMP/github" --json
  assert_rc "$RUN_RC" 2
  ...
  assert_rc "$RUN_RC" 0
  assert_has "$TMP/out" '"reason":"body-valid"'
  printf '%s\n' 'Refs AUT-281' 'Fixes AUT-281' >"$TMP/body"
  run_adapter "$TMP/out" validate AUT-281 --disposition stale --body-file "$TMP/body" --project "$TMP/linear" --json
  assert_rc "$RUN_RC" 2
  assert_has "$TMP/out" '"reason":"closing-nonfixed-issue"'
  ...
  echo "==> PASS: tracker adapter preserves claim/validation semantics"
)
