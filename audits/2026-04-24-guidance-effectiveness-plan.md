...
**Phase 2 — Claude hooks for hard rules.** Ship `branch-guard` + `emergency-disclosure` as `PreToolUse` hooks in `templates/claude-settings.json`. Add `tests/test-guidance-hooks.sh` covering: (a) triggering commands exit 2 with a useful message; (b) non-triggering commands pass through; (c) `TOUCHSTONE_EMERGENCY=1` overrides cleanly; (d) **latency budget — p95 of 20 hook invocations on a non-matching command stays under 100ms**, with the project-tracked target being 50ms p95 once measured. Update `bootstrap/update-project.sh` and `bootstrap/new-project.sh` to sync `.claude/settings.json`, with **a checksum check before overwrite** that warns when the user has hand-edited the platform-owned settings file (prevents silent loss of customizations). High-leverage, mechanically tested, no dependency on principle restructuring.

**Phase 3 — first new skill (`agent-swarms`).** The original ask. Build the skill-activation probe shape alongside it. Run probe **5× per trigger phrase across 5 phrasings (25 trials)** to bound activation rate with confidence, not eyeball it. Success criteria: (a) ≥80% activation rate across the 25 positive trials; (b) 0% on the 5 negative-control phrasings; (c) skill description fits the **~200-character cap** so it leaves headroom in the per-skill ~1536-char window Claude Code allots — anything longer becomes a link to a `principles/` reference. If activation falls below 80%, iterate the description and re-run before shipping.
