...
**Phase 5 — slim CLAUDE.md.** *Shipping now.* Replaces the `@principles/engineering-principles.md` full import with a 13-line named-rule summary (each rule = name + one-sentence essence + where applicable, a pointer to its activation skill). Full rationale stays in `principles/engineering-principles.md` — the file isn't deleted, just no longer auto-loaded into every session. CLAUDE.md drops from 130 lines (with @-import expansion ~210 effective) to 114 lines. Success criterion met: pushback probes for all three named rules (no-silent-failures, every-fix-gets-a-test, no-band-aids) still 4/4 pass against the slimmed file — the named summary is enough signal for Claude to apply the rule when challenged. Side benefit: the slim CLAUDE.md is now a stable prefix at the start of every session, ideal for prompt caching.
