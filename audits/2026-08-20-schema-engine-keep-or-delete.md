...
| --- | --- | --- | --- |
| Static command-start classifier (`declared_command_head`, `literal_words`, `declared_command_unrunnable_code`, `/usr/bin/env` option emulation) — ~495 lines, 46% of the engine | **Delete** | Inherited #802 corpus hardening; no failure cited for the static prediction itself. Verified today: both branches call `record_failure` and fail validation — the classifier only chooses the label `command-not-started` vs `command-failed`. `bash -c` already returns 126/127 at runtime for the same conditions. | Nothing. Run the command; derive the label from the real exit status (126/127 → not started). ~5 lines replace ~495, and the ~325-line test block shrinks to the exit-status cases. |
