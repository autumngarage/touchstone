...
5. **Reconcile issues.** Before opening the PR, list every GitHub issue found, claimed, fixed, partially fixed, or made stale by the work. Fully fixed issues get a GitHub-supported closing reference such as `Closes #123`; the legacy `Closes-issue:` claim alias does not auto-close. Partial/stale issues get a comment explaining the evidence or remaining gap. Do not leave fixed issues open silently.
