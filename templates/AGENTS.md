...
6. Hand the completed worktree to `touchstone worker ship --worktree "$PWD" --detach --review-fix`, record the status/takeover commands, and move to a disjoint batch.
7. The PR is the review surface. Confirm the detached owner reaches approved merge with `touchstone worker status --worktree "$PWD" --show-log`; use foreground `bash scripts/open-pr.sh --auto-merge` only for interactive debugging.
