...
**Review is always required.** The AI reviewer reports `COMMENTED`, not `APPROVED`, so approval count does not represent it; `review-gate` requires a trusted clean verdict for the exact head. Where that gate is absent, exact-head review remains mandatory driver procedure — a rollout gap, not permission to skip it.
...
4. **Change + commit.** Stage explicit files. Normal: `touchstone review check`, then `touchstone review run`; failure is a waiver, never fallback. Run at most one tier-required local AI pass per coherent unit, none for trivial work, and never rerun to confirm fixes. One concern per commit. Run `git show --stat --oneline HEAD`; unchanged `HEAD`: do not ship.
5. **Local review handoff.** Normal: `openrouter on staged: <n> findings`. Serious: save `git rev-parse HEAD`; run `touchstone review run --base <default>` once — Codex first, bounded fallback on any non-success; record `<reviewer> on <head-sha>: <n> findings, <disposition>`, waiving only if both are gone; never record the base. Hosted review owns exact heads.
...
9. **Merge.** After policy-bound exact-head success, run `touchstone pr merge <n> --head <sha>`. A queue entry ends mutation; never dequeue or probe admin merge. Verified queue enforcement permits raw `gh pr merge <n> --squash --match-head-commit <sha>`; otherwise stop. The merge-group gate owns later feedback. Confirm state.
