...
9. **Merge.** After exact-head success, `touchstone pr merge <n> --head <reviewed-sha>` queues or merges it and returns once accepted; never starts review. Raw recovery: verify gate success, then `gh pr merge <n> --squash --match-head-commit <sha>`. Confirm GitHub state despite exit codes.
