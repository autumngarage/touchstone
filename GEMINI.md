...
4. **Change + commit.** Stage explicit files. Normal: require readable `${CODEX_HOME:-$HOME/.codex}/review-normal.config.toml`, then run `codex -p review-normal review --uncommitted` before commit. Concise message; one concern per commit.
5. **Local review evidence.** Serious: run `codex review --base <default>` on the committed branch before push. Either tier records `codex on <target>: <n> findings, <disposition>` in the PR body's `- Local review:` row. Normal may waive for profile/pass or Codex unavailability; serious may waive only when Codex is unavailable. `delivery-evidence` refuses a missing, malformed, or unexplained row.
