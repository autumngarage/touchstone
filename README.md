...
`require_declared` is answered per action, not once per run: a `validate` that declared `lint_command` but nothing for `test` still fails, because one declared constituent does not vouch for the rest. In a `targets=` monorepo each target declares its own commands in its own `.touchstone-config`, so a monorepo is not exempt from the gate. That includes `validate_command`: a target that declares one has it dispatched directly, and only targets declaring none fall back to the constituent lint/typecheck/build/test actions. `TOUCHSTONE_RUN_REQUIRE_DECLARED=1` can turn the gate on for a project that has not declared it; it can never turn it off for one that has.

`touchstone doctor` is where those gaps become visible. It reports per-profile test presence, profile-specific linter availability (`ruff`, `swift-format`), pre-push hook integrity, and unknown profile values — in lock-step with the runner's dispatcher so doctor never claims more coverage than `validate` actually runs. A declared command suppresses the matching profile check at the level that declared it, root or target, because the run never reaches that profile default. Use `touchstone init --scaffold-tests` to seed a placeholder smoke test (Python, Node, and Go; Rust and Swift already get tests from `cargo init` / `swift package init`) and `touchstone init --ci github` to add a GitHub Actions workflow that runs the same `validate` path CI-side.
