...
`require_declared` is answered per action, not once per run: a `validate` that declared `lint_command` but nothing for `test` still fails, because one declared constituent does not vouch for the rest. In a `targets=` monorepo each target declares its own commands in its own `.touchstone-config`, so a monorepo is not exempt from the gate. `TOUCHSTONE_RUN_REQUIRE_DECLARED=1` can turn the gate on for a project that has not declared it; it can never turn it off for one that has.
