...
On-demand audit of `~/Repos/toolkit` — a shared engineering platform that propagates to every downstream project via `sync-all.sh`. This skill produces a structured audit report. **It never modifies toolkit files.** The user reviews the report and explicitly asks for follow-up changes in a separate turn.

The toolkit changes slowly but the ecosystem around it (Claude Code, Anthropic guidance, Codex, agent design patterns) changes quickly. The job of this audit is to surface drift in both directions: principles or implementation that have fallen behind current thinking, and new capabilities worth adopting.
...
- A one-paragraph statement of what the toolkit is and what it propagates.
...
- Anything the toolkit specifically uses: Codex CLI review workflows, Homebrew tap conventions, GitHub Releases automation

For each relevant finding, record internally: source URL, one-line summary, which toolkit file(s) or process it touches.
...
- Is the rule *missing* something a 2026-era agent/toolkit needs? Examples: prompt-injection defense for tool results, sub-agent coordination, context-window hygiene, skill-authoring conventions, hook lifecycle guarantees.
...
Spot-check (do not exhaustively audit — that's a separate task) the implementation for violations of the toolkit's *own* principles:
...
This pass evaluates the *workflows* the toolkit enforces, not just the code. For each workflow below, ask: *is this still the right shape? Should anything be added, refined, or retired?*
...
- **Release flow** (`lib/release.sh` + `bin/toolkit release` + Homebrew tap in `homebrew-toolkit/`) — the four-way agreement invariant (GitHub Releases, Homebrew, `origin/main`, local brew install). Anything new to add here? Any steps that are now redundant?
- **Bootstrap & propagation** (`bootstrap/new-project.sh`, `bootstrap/update-project.sh`, `bootstrap/sync-all.sh`) — the mechanism by which toolkit changes reach downstream projects. Are there Claude Code–native mechanisms (plugins, skills, hooks) that should replace or augment file-copy propagation?
- **Auto-update check** (`lib/auto-update.sh` and `~/.toolkit/last-update-check`) — is file-based update polling still the right pattern?
- **Testing discipline** (`tests/test-*.sh`) — is shell-based self-testing still the right approach, or would a different harness serve the propagation-sensitive nature of the toolkit better?
...
```markdown
# Toolkit Audit — YYYY-MM-DD

## Summary
- Toolkit version: vX.Y.Z
...
Anything from Phase 2 research that isn't drift per se but is new surface area the toolkit could adopt. One subsection per capability. Include what it is, where it would fit, and what it would replace or augment.
...
- **adopt** / **refine** / **add-capability** — a concrete change is recommended and fits the toolkit's philosophy
- **skip** — current guidance exists but contradicts a deliberate toolkit decision; name the principle that conflicts
...
- Don't propose changes to the downstream projects registered in `~/.toolkit-projects`. Scope is the toolkit repo only.
- Don't research non-Anthropic sources. No blogs, no Medium, no community repos.
- Don't write or edit any file outside `audits/`.
- Don't generate a punch-list of trivial style nits. The toolkit's principles are about structural quality.
