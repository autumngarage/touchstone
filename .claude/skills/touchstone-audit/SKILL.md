...
- **PR-visible review gate** (`scripts/open-pr.sh` + `scripts/merge-pr.sh` + `.touchstone-review.toml`) — exact-head/base review requests, trusted review evidence, unresolved-thread handling, and merge authorization. Does the gate fail closed without blocking migration from older policy files? Is review latency kept off the active coding path?
