...
3. Stage explicit file paths, then the tier's one local pass (`principles/local-review.md` — normal: `coderabbit review --agent --uncommitted` on the staged slice, before the commit; serious: `codex review --base <default>` on the branch, before the push); record it in the PR body's `- Local review:` row (reviewer, head, finding count, or `n/a — <reason>` for an absent/unauthenticated/exhausted CLI) — `delivery-evidence` refuses a normal/serious PR without it
4. Commit (concise message, one concern per commit) — deterministic gates first
