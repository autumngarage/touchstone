...
- **Issue-closing references follow the declared tracker.** `.touchstone-review.toml`'s `[issues].tracker` decides; undeclared means GitHub. Under **GitHub**: `Closes-issue: #123` in the commit body — `open-pr.sh` injects `Closes #123` into the PR body, auto-closing on merge. Under any **other tracker** (e.g. `linear`): no trailer, no injection — put the reference in the PR **body** yourself (`Fixes CON-123`), and keep GitHub's `#N` closing syntax out of your commit messages, which `open-pr.sh` refuses on both the create and the update path because `--squash` would close an untracked GitHub issue. Claim with `claim-issue.sh <ref>`; **exit 3 means nothing was claimed or verified** and you must do it by hand in the tracker.
