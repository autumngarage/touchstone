...
5. Serious: run `codex review --base <default>` at most once on the coherent committed branch before its first push. Either tier records `codex on <target>: <n> findings, <disposition>` in the PR body's `- Local review:` row; normal names the staged slice and may waive for a failed profile check, a failed configured pass, or Codex unavailability, while serious names the reviewed revision. Serious may waive only when Codex is unavailable. After allowed local findings are fixed, rerun deterministic checks and push; the hosted PR reviewer owns exact-head review for every pushed head. Another local AI pass is neither required nor authorized
