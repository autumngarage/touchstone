...
4. Stage explicit file paths; normal tier: `coderabbit review --agent --uncommitted -c principles/local-review-contract.md` on the staged slice *before* the commit; commit (concise message, one concern per commit) — deterministic gates first
5. Serious tier: `codex review --base <default>` on the committed branch *before* the push. Either tier records its pass in the PR body's `- Local review:` row (reviewer, head, finding count; or `n/a — <reason>` for an absent/unauthenticated/exhausted CLI) — `delivery-evidence` refuses a normal/serious PR without it or with the wrong reviewer for the tier
