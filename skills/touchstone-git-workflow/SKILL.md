...
4. Stage explicit files; normal: require `touchstone review check`, run `touchstone review run` on the isolated staged slice before commit, then commit one concern — deterministic gates first. A failed check is an explicit normal-tier waiver, never permission to fall back silently to the default profile
5. Serious: run `codex review --base <default>` on the committed branch before push. Either tier records `codex on <target>: <n> findings, <disposition>` in the PR body's `- Local review:` row; normal names the staged slice and may waive for a failed profile check, a failed configured pass, or Codex unavailability, while serious names the reviewed revision. Serious may waive only when Codex is unavailable
