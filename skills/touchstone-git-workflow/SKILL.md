...
3. Claim tracked work before implementation — GitHub: `touchstone tracker claim <ref>`; Linear: assign yourself and re-read the assignee. Claim every item in a bundle
4. Stage explicit files; normal: require `touchstone review check`, run `touchstone review run` on the isolated staged slice before commit, then commit one concern — deterministic gates first. A failed check or run is an explicit normal-tier waiver, never permission to fall back to an unbounded model path. A tier-required local AI pass runs at most once per coherent review unit and is never rerun to confirm fixes; trivial work initiates none
5. Serious: capture `reviewed_head="$(git rev-parse HEAD)"`, then run `touchstone review run --base <default>` at most once on that coherent committed branch before its first push. It runs Codex and falls back to the bounded OpenRouter pass over the same branch on any Codex non-success, including an exhausted quota. Record normal as `openrouter on the staged slice (review-normal): <n> findings, <disposition>` or serious as `<reviewer> on <captured-head-sha>: <n> findings, <disposition>` for whichever reviewer it reported, in the PR body's `- Local review:` row. The serious SHA is the captured current head the pass reviewed; `<default>` is only the comparison boundary. Normal may waive for a failed configured check or pass; serious may waive only when Codex and the fallback are both unavailable. After allowed local findings are fixed, rerun deterministic checks and push; the hosted PR reviewer owns exact-head review for every pushed head. Another local AI pass is neither required nor authorized
6. Reconcile tracked work: list every tracker item found, claimed, fixed, partially fixed, or made stale; fixed items get the closing reference in the PR body, partial or stale ones a tracker note
7. `git push -u origin HEAD`, then `touchstone pr open --expect-branch <branch> --title … --body-file …` — the installed CLI is the sequencer everywhere: it creates or reuses the PR, posts the request, and confirms the gate bound it to the exact head and base. Put the configured close (`Closes #123` or `Fixes AUT-123`) in the PR body, not only a commit. Request review through `pr open`, not by hand; re-run it for any later head (idempotent). Never put its marker in a comment you write yourself; a bare `@codex review` from a collaborator is valid only in recovery — bounded stalled-request recovery, or the CLI-absent raw sequence (`gh pr create`, the bare comment, re-run the gate) — never the instruction
8. Inspect GitHub's complete review surface; answer every finding but implement only the ones the reviewer badged P0 or P1 — route every P2, P3, and unbadged finding. The badge is the bar; the reviewer's categories are its admission criteria, not its severity scale, so never re-derive severity from the prose. If a review fix creates a defect, revert or simplify before another mutation and follow the cascade rule below. Exact-head review after any allowed fix commit is never skipped. Follow the raw reply and GraphQL `resolveReviewThread` procedure in `principles/git-workflow.md`
9. After the policy-owned exact-head gate succeeds, run `touchstone pr merge <n> --head <reviewed-sha>` (queues or merges only that head and returns after GitHub accepts it; it never starts review); raw fallback verifies the exact-head gate, then runs `gh pr merge <n> --squash --match-head-commit <reviewed-sha>`; confirm `state == MERGED`
10. Clean up locally (`git branch -d <feature>`, or `-D` after confirming the content landed)
...
- **Review is always required.** Where installed and verified as required, `review-gate` under gate behavior contract 3 passes only on a trusted clean verdict for the exact head (under behavior v2 it required exact-head evidence plus answers to every finding), while GitHub conversation resolution independently requires every inline thread closed. Without those gates, follow the same review loop and treat missing enforcement as an adoption gap.
...
- **Review cannot amend the approved scope; answering is not implementing.** Fix diff-created regressions and violations of recorded acceptance criteria *that the reviewer badged P0 or P1*. A P2 or P3 is answered and routed, never fixed in the PR that received it — correct, one-line, already-in-the-diff, and cheaper-than-answering are rationalizations, not exceptions. Answer and route real out-of-scope findings without changing the PR. Stop only widened work and requests on that shape; in-scope fixes continue to exact-head review. Then narrow, split along independently approved criteria, or redesign.
- **A review-fix regression is a stop signal.** On the first, revert or simplify to the last known-good design and audit the class before another mutation; its regression test is evidence, not permission to retain the failed fix. On the second, or the third fix round, end same-shape mutation and genuinely split or close and replan. Exact-head review remains mandatory for any head that might merge, but never authorizes more mutation after the stop.
- **One ordinary review request per exact head-and-base binding.** The binding is head SHA, base ref, and base SHA. Under gate behavior contract 3 the gate passes only on a later trusted clean verdict: resolve thread-backed findings and let `touchstone pr answer` post the one idempotent attest request its resolution of the last thread triggers, then merge on the clean verdict. Where only behavior v2 is effective, answered findings satisfy that gate (issue #751): resolve and merge. Four cases permit another request while the head stays unchanged (counting that attest): the base ref or base SHA changed after the earlier request is completed or explicitly failed, one body-only-finding request, or the single audited provider-recovery trigger below. If the base changes while the earlier request is nonterminal, wait or integrate the current base into the branch to produce a genuinely new head before requesting review; results identify the head, not their request or base. Never manufacture an empty commit to refresh review. A fix commit moves the head: push it and request its one review for the current base.
...
- **Round budget: three fix rounds per capability, never more than three on one PR.** A fix round is one push of review-driven change, recorded in the PR body's `Review budget` row as it is pushed and never reconstructed from `git log`, which amend, squash, and rebase all rewrite — the budget counts mutation, never requests, because findings arrive on surfaces the reviews API does not fully expose and because the contract-3 attest carries no change and so consumes no budget. After the third fix round, do not push a fourth on the same implementation shape. Closing, renaming, restacking, or mechanically splitting the same acceptance criterion does not reset its count. After exhaustion, record the root cause and narrow the acceptance boundary or replace the architecture around a class-level guardrail before requesting review again. That redesigned attempt gets one validation round; another finding closes or genuinely decomposes the capability. Merge-if-answered applies only when no known P0/P1 defect remains. Do not resume one-finding-at-a-time expansion.
...
- **A live exact-head queue entry receives zero more merge mutations.** Wait for GitHub; never dequeue, re-enqueue, cancel the merge-group run, or probe an admin merge. Urgency or frustration is not bypass authorization: the human must explicitly authorize bypassing normal policy, and effective policy plus `mergeQueueEntry` are inspected read-only before any attempt.
