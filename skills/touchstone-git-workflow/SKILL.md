...
5. Serious: capture `reviewed_head="$(git rev-parse HEAD)"`, then run `codex review --base <default>` at most once on that coherent committed branch before its first push. Record normal as `openrouter on the staged slice (review-normal): <n> findings, <disposition>` or serious as `codex on <captured-head-sha>: <n> findings, <disposition>` in the PR body's `- Local review:` row. The serious SHA is the captured current head the pass reviewed; `<default>` is only the comparison boundary. Normal may waive for a failed configured check or pass; serious may waive only when Codex is unavailable. After allowed local findings are fixed, rerun deterministic checks and push; the hosted PR reviewer owns exact-head review for every pushed head. Another local AI pass is neither required nor authorized
