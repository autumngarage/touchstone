...
- **An AI reviewer never approves.** GitHub reserves `APPROVED` for real users, so the gate is an answered review bound to the current head — not an approval count. Do not wait for one.
