...
4. Stage explicit files; normal: require `touchstone review check`, run `touchstone review run` on the isolated staged slice before commit, then commit one concern — deterministic gates first. A failed check is an explicit normal-tier waiver, never permission to fall back silently to the default profile. A tier-required local AI pass runs at most once per coherent review unit and is never rerun to confirm fixes; trivial work initiates none
5. Serious: capture `reviewed_head="$(git rev-parse HEAD)"`, then run `codex review --base <default>` at most once on that coherent committed branch before its first push. Record normal as `codex on the staged slice (review-normal): <n> findings, <disposition>` or serious as `codex on <captured-head-sha>: <n> findings, <disposition>` in the PR body's `- Local review:` row. The serious SHA is the captured current head the pass reviewed; `<default>` is only the comparison boundary. Normal may waive for a failed profile check, a failed configured pass, or Codex unavailability; serious may waive only when Codex is unavailable. After allowed local findings are fixed, rerun deterministic checks and push; the hosted PR reviewer owns exact-head review for every pushed head. Another local AI pass is neither required nor authorized
