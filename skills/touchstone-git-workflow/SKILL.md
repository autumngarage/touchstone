...
4. Stage explicit files; normal: require readable `${CODEX_HOME:-$HOME/.codex}/review-normal.config.toml`, run `codex -p review-normal review --uncommitted` on the isolated staged slice before commit, then commit one concern — deterministic gates first
5. Serious: run `codex review --base <default>` on the committed branch before push. Either tier records `codex on <target>: <n> findings, <disposition>` in the PR body's `- Local review:` row; normal names the staged slice and may waive for profile/pass or Codex unavailability, while serious names the reviewed revision. Serious may waive only when Codex is unavailable
