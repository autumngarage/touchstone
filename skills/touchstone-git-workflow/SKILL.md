...
4. Stage explicit files; normal: require `touchstone review check`, run `codex -p review-normal review --uncommitted` on the isolated staged slice before commit, then commit one concern — deterministic gates first. A failed check is an explicit normal-tier waiver, never permission to fall back silently to the default profile
