...
- **Round budget: three per PR.** A discipline now, not an enforced limit — the wrapper that refused a fourth request is gone. After a third finding-bearing review, do not post a fourth request on the same implementation shape. Merge if answered, or materially narrow, split, or redesign around a class-level guardrail and state the evidence in the PR. If the redesigned head produces another finding-bearing round, split or close the PR while preserving the corpus; do not resume one-finding-at-a-time expansion.
