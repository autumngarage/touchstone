...
5. Serious: run `codex review --base <default>` on the committed branch before push. Either tier records `codex on <target>: <n> findings, <disposition>` in the PR body's `- Local review:` row, or `n/a — <reason>` when its profile/pass or Codex is unavailable; normal names the staged slice, serious the reviewed revision
