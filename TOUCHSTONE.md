...
9. **Merge.** After policy-bound exact-head success, run `touchstone pr merge <n> --head <sha>`. An exact-head queue entry ends mutation—never dequeue/re-enqueue or probe admin merge. Raw recovery may run `gh pr merge <n> --squash --match-head-commit <sha>`; the merge-group gate owns later review activity. Confirm state.
