...
5. **Local review evidence.** Serious: run `codex review --base <default>` on the committed branch before push. Either tier records `codex on <target>: <n> findings, <disposition>` in the PR body's `- Local review:` row, or `n/a — <reason>` when its profile/pass or Codex is unavailable. `delivery-evidence` refuses a missing, malformed, or unexplained row.
