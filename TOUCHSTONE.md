...
5. **Local review handoff.** Serious: save `git rev-parse HEAD`; run `codex review --base <default>` once. In `- Local review:`, record `codex on <head-sha>: <n> findings, <disposition>` or Codex-unavailable waiver; never record the base. Test fixes; push. Hosted review owns exact heads.
