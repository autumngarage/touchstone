...
- **Stop when the task is correct, not when review runs out of remarks** — deterministic gates first, then review. Implement only high-severity findings: correctness, crashes, data loss, security, broken behaviour, unacceptable performance, lifecycle failures. Answer and route the rest; never reopen the design space. `principles/git-workflow.md` caps the rounds. **Exact-head review after a fix commit is never skipped** — this bounds what you implement, never what gets reviewed.
...
4. **Change + commit.** Stage explicit file paths. Normal tier: `coderabbit review --agent --uncommitted -c principles/local-review-contract.md` on the staged slice *before* the commit. Concise message. One concern per commit.
5. **Local review evidence.** Serious tier: `codex review --base <default>` on the committed branch *before* the push. Either tier records its pass in the PR body's `- Local review:` row — reviewer, head, finding count (`codex on abc1234: 3 findings, 2 fixed`) — or `n/a — <reason>` only when the reviewer CLI is absent, unauthenticated, or out of quota. `delivery-evidence` refuses a normal/serious PR whose row is missing, names the wrong reviewer, or waives without a reason.
...
9. **Merge.** `touchstone pr merge <n> --head <reviewed-sha>` re-runs the pinned gate and merges bound to that head (raw recovery: `gh pr merge <n> --squash --match-head-commit <sha>`). Confirm GitHub state regardless of exit code.
10. **Clean up before the session ends.** Checkout on the default branch at origin; your worktree, local branch, and merged remote branch gone; no untracked residue; tracker item in its terminal state. `touchstone cleanup check` must report nothing.
