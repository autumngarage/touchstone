...
4. **Change + commit.** Stage explicit files. Normal: require readable `${CODEX_HOME:-$HOME/.codex}/review-normal.config.toml`, then run `codex -p review-normal review --uncommitted` before commit. One concern per commit. Run `git show --stat --oneline HEAD`; unchanged `HEAD`: do not ship.
