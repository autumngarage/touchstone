...
3. **Claim tracked work before implementation.** GitHub tracker: `touchstone tracker claim <ref>` (race-safe, verified by re-read). Linear tracker: the adapter has no transport and says so — assign yourself through the Linear MCP and re-read the assignee before editing. Claim every item in a bundle so two agents do not ship competing fixes; an unavailable transport is unverifiable, never success.
4. **Change + commit.** Stage explicit file paths. Concise message. One concern per commit. The tier's local pass runs first (normal: staged, pre-commit; serious: branch, pre-push).
...
8. **Merge.** `touchstone pr merge <n> --head <reviewed-sha>` re-runs the pinned gate and merges bound to that head (raw recovery: `gh pr merge <n> --squash --match-head-commit <sha>`). Confirm GitHub state regardless of exit code.
