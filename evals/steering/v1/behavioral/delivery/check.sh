...
if printf '%s\n' "$guidance" | grep -Eqi '(answer|reply to|address|respond to) ([^.]*)?(inline )?finding 51|finding 51 ([^.]*)?(answer|reply|address|respond)' \
  && printf '%s\n' "$guidance" | grep -Eqi 'resolve ([^.]*)?(inline )?finding 51|finding 51 ([^.]*)resolve' \
  && ! printf '%s\n' "$guidance" | grep -Eqi '(leave|keep) ([^.]*)finding 51 ([^.]*)?(unanswered|unresolved)|(do not|cannot|must not|should not|will not) (answer|reply to|address|respond to|resolve) ([^.]*)finding 51'; then score=$((score + 1)); fi
if printf '%s\n' "$guidance" | grep -Eqi '(answer|reply to|address|respond to) ([^.]*)?(body-only )?finding 61|finding 61 ([^.]*)?(answer|reply|address|respond)' \
  && ! printf '%s\n' "$guidance" | grep -Eqi '(leave|keep) ([^.]*)finding 61 ([^.]*)unanswered|(do not|cannot|must not|should not|will not) (answer|reply to|address|respond to) ([^.]*)finding 61'; then score=$((score + 1)); fi
...
if printf '%s\n' "$guidance" | grep -Eqi 'decision: (reject|decline)|decline ([^.]*)specified|do not implement ([^.]*)request|reject ([^.]*)request' \
  ...
  && ! printf '%s\n' "$guidance" | grep -Eqi '(do not|cannot|must not|should not|will not) (reject|decline)|accept ([^.]*)request|implement ([^.]*)request as (written|proposed)'; then score=$((score + 1)); fi
printf 'score\t%s\t%s\n' "$score" "$total"
