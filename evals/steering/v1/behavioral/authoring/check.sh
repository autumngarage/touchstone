...
repo="$1" events="$2" baseline="${3:-}" score=0 total=6 mutant="" facts=""
cleanup() {
...
bash "$(dirname "$0")/../../normalize-events.sh" "$events" >"$facts"
branch_before_edit() {
  awk -F '\t' '
    $1 == "branch" && (!branch || $2 < branch) { branch=$2 }
    $1 == "edit" && (!edit || $2 < edit) { edit=$2 }
    END { exit !(branch && edit && branch < edit) }
  ' "$facts"
}
consulted_preimplementation() {
  grep -qFx 'checklist-read' "$facts"
}
