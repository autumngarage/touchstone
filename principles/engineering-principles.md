...
Before a driving CLI initiates or dispatches another edit, it must stop scope expansion when two consecutive review/fix cycles reveal new structural defects or when review expands the work to another independently shippable concern. Preserve the branch, record the stop state in the PR or issue, and split or replan. An already-running autonomous worker needs separate orchestration enforcement; do not infer that agent guidance controls its inner iteration count.

Scope never excuses a regression introduced by the current diff. After the stop, the only edits allowed on the branch are fixes or reverts for current-diff correctness, security, or data-integrity blockers. File pre-existing independent findings without claiming them until implementation starts, then resume with the smallest shippable concern.
