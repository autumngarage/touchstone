...
## Bound delivery loops
Define the smallest shippable concern before implementation: one invariant, the minimum coupled surfaces required to ship it, focused validation, explicit non-goals, and a stop condition. Review improves that bounded change; it does not authorize indefinite scope growth. Freeze and split or replan when review reveals another independently shippable concern, the changed-file surface expands beyond that minimum, or two consecutive review/fix cycles uncover new structural defects.

A request to "ship everything" sets priority and sequencing, not PR size. File valid findings outside the current invariant, but claim them only when implementation starts. A delivery platform that consumes more time than the customer change it enables has crossed its stop condition and must be simplified or deferred.
