...
Define the smallest shippable concern before implementation: one invariant, one owned subsystem, focused validation, explicit non-goals, and a stop condition. Review improves that bounded change; it does not authorize indefinite scope growth. Freeze and split or replan when review reveals a second subsystem, the changed-file surface materially expands, or two consecutive review/fix cycles uncover new structural defects.

A request to "ship everything" sets priority and sequencing, not PR size. Valid findings outside the current invariant become claimed follow-up work. A delivery platform that consumes more time than the customer change it enables has crossed its stop condition and must be simplified or deferred.
