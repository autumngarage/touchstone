...
5. **Local review evidence.** Serious tier: run `codex review --base <default>` on the committed branch before push. This is the one step nothing else witnesses (AUT-443), so either tier records `codex on <target>: <n> findings, <disposition>` in the PR body's `- Local review:` row, or `n/a — <reason>` when its profile/pass or Codex is unavailable. Normal names the staged slice; serious names the reviewed revision. `delivery-evidence` refuses a missing, malformed, or unexplained row.
