...
- Touchstone's only model-routing decision is the bounded normal local-review cost lane. `touchstone review setup` installs a managed Codex profile that uses OpenRouter Pareto Code with a Keychain-backed credential; the serious local pass and PR-side review keep their default Codex paths. Local review informs the driver and never gates a merge — GitHub's PR-visible review remains the only semantic authority.
