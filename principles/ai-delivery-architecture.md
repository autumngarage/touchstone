...
- Touchstone's only model-routing decision is the bounded normal local-review cost lane. The stable `touchstone review` command uses a versioned policy and one direct OpenRouter request over the staged diff; OpenRouter Pareto Code selects the cheapest model meeting the configured coding floor and absolute provider-price ceilings. The serious local pass and PR-side review keep their default Codex paths. Local review informs the driver and never gates a merge — GitHub's PR-visible review remains the only semantic authority.
