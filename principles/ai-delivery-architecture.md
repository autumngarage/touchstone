...
Agentic PR Review Loop
  ...
  | - the head merges on a trusted clean exact-head verdict (the
  |   answer flow requests it when the last thread resolves); each
  |   gate run evaluates once, and the driver waits for the review
  |   on its own machine, then re-runs the gate
  |
  v
Merge Gate
  |
...
- Touchstone initiates no local AI review. Review is the PR-visible reviewer's, on every pushed head: where the repository's effective policy requires `review-gate`, the primary reviewer is asked first, and when it is at capacity the pinned gate reviews the head itself through its fallback reviewer — a bounded OpenRouter request whose instructions are Touchstone's `config/review-normal-prompt.md`, fetched at a pinned revision and checksum. That fallback is Touchstone's only model-routing decision, and GitHub's PR-visible review remains the only semantic authority.
...
- Feature-branch push is not the merge gate. The contract puts no full test suite or LLM review in front of it — local hooks stay cheap feedback — because the complete checks and the review run on GitHub against the pushed head, on hosted or self-hosted runners. Where effective policy lacks the protected validation workflow, the complete suite runs locally until that rollout gap closes.
- Merge is allowed only after PR-visible review and check approval, and the gate behavior contract decides what that means. Under version 4, the one Touchstone's policy declares, the gate requires one trusted, unedited clean verdict bound to the exact head, evaluates it once per run, and never adjudicates answer history, while GitHub independently requires every inline review thread resolved. Read the repository's effective policy rather than assuming it.
...
- **The configured AI reviewer reports `COMMENTED`, not `APPROVED`.**
  ...
  that check requires a trusted clean verdict bound to the exact head. Until
  adoption, the same exact-head review remains
