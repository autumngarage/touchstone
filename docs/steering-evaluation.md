...
Provider event schemas cross one versioned normalization boundary before a
scenario assigns credit. The normalizer correlates tool requests with
successful results and emits provider-neutral facts for branch position,
edits, checklist reads, and validation. Ordering facts retain positions within
a single shell command. A conjunctive rubric point is earned only when every
required action has independent evidence; omission and opposite-action fixtures
