...
the same plan as dry-run. An apply with planned changes is permitted only from
a clean, non-default branch; a no-op current result performs no write and does
not use that mutation precondition as a safety-state check.
...
worktree safety precondition; and 6 means an operational failure. Exit 6 can
also follow completed writes when transaction-artifact cleanup fails, so
callers must inspect the planned paths before retrying.
...
an accepted apply produces no second diff after its generated artifacts are
committed.

Repository facts that decide generated commands or setup must be tracked.
A successful fresh apply creates reviewable, initially untracked outputs;
commit them before another plan. An existing contract or Touchstone-managed
output that is present but untracked is refused because it would disappear
from a fresh checkout. Any apply that still has changes retains the
