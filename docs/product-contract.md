...
| Require review of the exact PR head | GitHub required `review-gate` workflow | Check name and versioned evidence contract; evaluator verdicts distinguish `waiting-request`, `waiting-review`, terminal `failure`, and `success` while every non-success conclusion remains fail-closed; the optional UTC `evidenceCutoffAt` input evaluates only evidence observed at or before a workflow-owned deadline | No-review can remain pending only while the authoritative evaluator reports a waiting state; cutoff fixtures reject evidence posted or edited after a deadline; stale-head, moved-base, malformed evidence, and API-failure fixtures fail immediately; a quota notice remains provisional non-evidence while the driver continues waiting |
