...
| Route normal local review through the lower-cost lane | The installed tool, machine-wide | Managed `review-normal` Codex profile plus command-backed macOS Keychain credential access | Offline setup/check/rotate/uninstall fixtures prove per-home isolation, idempotency, backup/restore, rollback, no secret in config or output, drift refusal, and fail-closed missing-credential behavior |
