...
| Require review of the exact PR head | GitHub required `review-gate` workflow | Check name and versioned gate behavior contract (version 4): the evaluator derives one normalized verdict — `waiting`, `findings`, `clean`, or `invalid` — for the current head, and only a trusted, unedited, explicit clean result bound to that head succeeds; each run evaluates once and never waits, the driver waiting for the reviewer on its own machine and then re-running it; the fallback reviewer stands in only past the evidence deadline, after the reviewer says it cannot answer, or on a bot-authored pull request, and never over a completed findings verdict; evidence collection is bounded O(pages of current surfaces), independent of review-history size | A pushed head inherits no verdict; stale-head, edited, ambiguous, simultaneous, unresolvable-abbreviation, malformed-timestamp, and missing-retarget-evidence fixtures fail closed; a quota notice or unbound provider error is never itself review evidence, and a waiting run fails rather than waits |
| Require every review finding to be answered | Driver procedure plus GitHub conversation resolution | Recorded disposition markers and the sequencer's answer flow; resolving the last open thread posts exactly one idempotent fresh review request for the clean verdict, waits locally for it, and re-runs the gate once | An unresolved thread blocks natively; the gate itself never adjudicates answer history — it requires the later clean exact-head verdict instead |
| Require inline review threads to be resolved | GitHub conversation resolution | GitHub review thread state | An unresolved thread blocks even after a reply |
