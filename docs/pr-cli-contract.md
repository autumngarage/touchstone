...
review evidence and never a blocker — and never a wait either: under gate
behavior contract 4 the notice ends `open`'s local wait, and the re-run pinned
`review-gate` reviews that exact head itself, so what
