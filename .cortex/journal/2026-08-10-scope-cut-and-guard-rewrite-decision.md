...
Both numbers were point-in-time observations, made at the start of 2026-08-09 and at the end of 2026-08-10. A future auditor counting the open backlog should run `gh issue list --state open --limit 200 | wc -l` — the explicit `--limit` matters, because `gh issue list` truncates at a default of 30. The observations themselves predate this correction and the exact invocations were not preserved, so the counts stand as what was seen at the time, not as reproducible output of the command above, and this entry does not claim they can be re-derived.

That distinction is deliberate, and four rounds of review on this paragraph are what established it — the last round caught that the command this entry itself quoted as the source of the counts could not have produced them, because it omitted `--limit`. Reconstructing the transition after the fact would require netting closures, reopenings, and creations while excluding pull requests from all three, against an events endpoint that mixes issues and PRs and reports current state rather than state-at-the-time. Every version of that query I wrote was wrong in a different direction — it omitted the reopened #593, or subtracted PR #706's closure from an issue count, or let an issue closed *after* the window stop contributing when rerun.
